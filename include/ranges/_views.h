...
namespace ox {
    namespace ranges {
        ...
        public:
            ...
            public:
                iterator() = default;
                iterator(base_itr itr) : current(itr) {};
                ...
                requires std::equality_comparable<base_itr> {
                    return x.current == y.current;
                }

                friend auto operator<=>(const iterator &x, const iterator &y)
                requires std::ranges::random_access_range<R> && std::three_way_comparable<base_itr> {
                    return x.current <=> y.current;
                }
        ...
        public:
            ...
            explicit iterators_view(R base) : range(base), _iter(std::begin(range)) {};
        ...
        namespace details {
            struct iterators_range_adaptor_closure {
                constexpr iterators_range_adaptor_closure() = default;
            ...
            template<std::ranges::viewable_range R>
            constexpr auto operator|(R &&r, iterators_range_adaptor_closure const &a) {
            ...
            struct iterators_range_adaptor {
                ...
                constexpr auto operator()(R &&r, std::iter_difference_t<std::ranges::iterator_t<R>> count) {
                    return iterators_range_adaptor(std::forward<R>(r), count);
                }

                constexpr auto operator()() {
                    return iterators_range_adaptor_closure();
                }
            };
        }
